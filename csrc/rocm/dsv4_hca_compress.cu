...
constexpr int RATIO = 128;
constexpr int K_POOL = RATIO;          // no overlap -> K_POOL == ratio
constexpr int STATE_WIDTH = HEAD_SIZE; // no overlap: kv | score each HEAD_SIZE
constexpr int V0_BLOCK_SIZE = 256;     // v0: 4 waves (4 tokens/block)
...
// ============================================================================
extern "C" __global__ void hca_compress_kernel(
    int num_tokens,
    const __hip_bfloat16* __restrict__ state_cache,
    int64_t state_stride0, int64_t state_stride1,
    ...
    const int32_t* __restrict__ block_table, int64_t block_table_stride,
    int32_t block_size,
    const float* __restrict__ rms_norm_weight, float rms_norm_eps,
...
hca_compress_ksplit_kernel(
    ...
    if (((position + 1) & (RATIO - 1)) != 0 || kv_slot_idx < 0) return;
    if (slot_mapping[token_idx] < 0) return;
    hca_ksplit_body<NW>(token_idx, position, kv_slot_idx, state_cache,
        state_stride0, state_stride1, ape, ape_stride, token_to_req_indices,
        block_table, block_table_stride, block_size, rms_norm_weight, rms_norm_eps,
...
static void ksplit_launch(
    ...
    hipLaunchKernelGGL(
        hca_compress_ksplit_kernel<NW>, grid, block, 0, stream,
...
static void v0_launch(
    ...
    hipLaunchKernelGGL(
        hca_compress_kernel, dim3(num_blocks), dim3(V0_BLOCK_SIZE), 0, stream,
        num_tokens, state_cache, state_stride0, state_stride1,
        ape, ape_stride, token_to_req_indices, positions, slot_mapping,
        block_table, block_table_stride, block_size,
        rms_norm_weight, rms_norm_eps, cos_sin_cache, cos_sin_stride,
...
static void plan_ksplit_launch(
    ...
    hipLaunchKernelGGL(
        hca_compress_plan_ksplit_kernel<NW>, dim3(plan_capacity), dim3(WARP_SIZE * NW),
...
#define HCA_FWD_ARGS \
    ...
    block_table_stride, block_size, rms_norm_weight, rms_norm_eps, \
...
#define HCA_PLAN_ARGS \
    ...
    rms_norm_weight, rms_norm_eps, cos_sin_cache, cos_sin_stride, kv_cache, \
