...
constexpr int K_POOL = 8;
constexpr int HEAD_SIZE = 128;
constexpr int ROPE_HEAD_DIM = 64;
constexpr int NOPE_HEAD_DIM = HEAD_SIZE - ROPE_HEAD_DIM;  // 64
constexpr int STATE_WIDTH = 256;   // overlap: coff*head_dim = 2*128

constexpr int BLOCK_SIZE = 256;  // 4 waves
constexpr int WARP_SIZE = 64;    // AMD wavefront
constexpr int DIMS_PER_LANE = HEAD_SIZE / WARP_SIZE;  // 2

constexpr float FP8_MAX = 448.0f;
constexpr float INV_FP8_MAX = 1.0f / FP8_MAX;
constexpr float FP4_MAX = 6.0f;          // E2M1 max
constexpr float INV_FP4_MAX = 1.0f / FP4_MAX;

// Quant-format specializations.
constexpr int QFMT_FP8 = 0;
constexpr int QFMT_MXFP4 = 1;
...
__global__ void indexer_compress_kernel(
    ...
    #pragma unroll
    for (int d = 0; d < DIMS_PER_LANE; d++) {
        normed[d] = comp[d] * rrms * rms_norm_weight[dim_base + d];
    }
...
extern "C" void launch_indexer_compress(
    ...
    if (use_fp4_cache) {
        hipLaunchKernelGGL((indexer_compress_kernel<QFMT_MXFP4>),
                           grid, block, 0, stream, IDX_FWD_ARGS);
    } else {
        hipLaunchKernelGGL((indexer_compress_kernel<QFMT_FP8>),
                           grid, block, 0, stream, IDX_FWD_ARGS);
    }
    #undef IDX_FWD_ARGS
}
