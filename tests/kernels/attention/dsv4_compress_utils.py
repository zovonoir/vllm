...
INDEXER_MXFP4 = ShapeConfig("indexer_mxfp4", 128, 64, 4, True,
                            quant_format="indexer_mxfp4")
...
class CompressorContext:
    ...
    def build(self) -> "CompressorContext":
        ...
        kv_raw = torch.randn(self.num_tokens, shape.coff * shape.head_dim,
                             dtype=torch.bfloat16, generator=g) * 0.5
        score_raw = torch.randn(self.num_tokens, shape.coff * shape.head_dim,
                                dtype=torch.bfloat16, generator=g) * 0.5
        self.ape = torch.randn(shape.ratio, shape.coff * shape.head_dim,
        ...
        self.rms_weight = (torch.rand(shape.head_dim, dtype=torch.float32, generator=g)
                           * 0.5 + 0.5).to(torch.bfloat16)

        max_cos_pos = self.max_position + 1
        inv_freq = 1.0 / (10000 ** (torch.arange(0, shape.rope_head_dim, 2,
                                                  dtype=torch.float32) / shape.rope_head_dim))
        ...
        self.bytes_per_block = (KV_BLOCK_SIZE * shape.token_stride
                                + KV_BLOCK_SIZE * shape.scale_dim)
    ...
    def _populate_state_caches(self):
        ...
        save_partial_states(kv=self._raw["kv"], score=self._raw["score"], ape=self.ape,
                            positions=self.positions_t, state_cache=self.state_cache_fp32,
                            slot_mapping=self.slot_mapping_t, block_size=sbs,
                            state_width=shape.state_width, compress_ratio=shape.ratio)
        self.state_cache_bf16 = torch.zeros(
            self.max_state_blocks, sbs, 2 * shape.state_width, dtype=torch.bfloat16)
        save_partial_states(kv=self._raw["kv"], score=self._raw["score"], ape=None,
                            positions=self.positions_t, state_cache=self.state_cache_bf16,
                            slot_mapping=self.slot_mapping_t, block_size=sbs,
                            state_width=shape.state_width, compress_ratio=shape.ratio)
...
def _prefill(shape, name, seqlen):
    return _mk(shape, name, list(range(seqlen)), [0] * seqlen, list(range(seqlen)), seqlen)
...
def _multi(shape, name, lengths):
    ...
    for bi, L in enumerate(lengths):
        for p in range(L):
            P.append(p); T.append(bi); S.append(bi * ml + p)
...
def _decode(shape, name, positions, ctx_len=4096):
    ...
    for bi, mp in enumerate(positions):
        for p in range(mp + 1):
            P.append(p); T.append(bi); S.append(bi * ctx_len + p)
...
def build_shared_input(ctx: CompressorContext) -> None:
    ...
    for r, mn in req_min.items():
        for p in range(max(0, mn - window_depth), mn):
            if (r, p) in present:
                continue
            lb_pos.append(p); lb_slot.append(p); lb_req.append(r)

    bf16 = torch.zeros(ctx.max_state_blocks, sbs, 2 * sw, dtype=torch.bfloat16)
    save_partial_states(kv=ctx._raw["kv"], score=ctx._raw["score"], ape=None,
                        positions=ctx.positions_t, state_cache=bf16,
                        slot_mapping=ctx.slot_mapping_t, block_size=sbs,
                        state_width=sw, compress_ratio=ratio)
    if lb_pos:
        g = torch.Generator(device="cuda").manual_seed(20240611 + ctx.num_tokens)
        n_lb = len(lb_pos)
        lb_kv = torch.randn(n_lb, shape.coff * shape.head_dim, dtype=torch.bfloat16, generator=g) * 0.5
        lb_score = torch.randn(n_lb, shape.coff * shape.head_dim, dtype=torch.bfloat16, generator=g) * 0.5
        save_partial_states(kv=lb_kv, score=lb_score, ape=None,
                            positions=torch.tensor(lb_pos, dtype=torch.int64),
                            state_cache=bf16,
                            slot_mapping=torch.tensor(lb_slot, dtype=torch.int64),
                            block_size=sbs, state_width=sw, compress_ratio=ratio)
    ...
    for p, sl in list(zip(ctx.positions, ctx.slot_mapping)) + list(zip(lb_pos, lb_slot)):
        if sl < 0:
            continue
...
def run_triton(ctx, kv_3d) -> None:
    ...
    compress_norm_rope_store_triton(state_cache=ctx.state_cache_fp32, kv_cache=kv_3d,
                                    **ctx.generic_kwargs())
...
def run_hip(ctx, kv_3d) -> None:
    ...
    s = ctx.shape
    common = (
        ctx.state_cache_bf16, ctx.num_tokens, ctx.ape,
        ctx.token_to_req_t, ctx.positions_t, ctx.slot_mapping_t, ctx.block_table,
        s.state_block_size, ctx.rms_weight.to(torch.float32), RMS_EPS,
        ctx.cos_sin_cache, kv_3d, ctx.kv_slot_mapping_t, kv_3d.shape[1],
        s.scale_dim,
    )
    if s.head_dim == 128:               # indexer (FP8 or MXFP4)
        ...
        torch.ops._rocm_C.dsv4_hca_compress(*common)
    else:                               # CSA
...
def _dequant_h512(buf, ci):
    ...
    sc = buf[blk][soff + p * H512_SCALE_DIM:soff + p * H512_SCALE_DIM + H512_N_NOPE_BLOCKS]
    out = torch.zeros(512)
    for nb in range(H512_N_NOPE_BLOCKS):
        e = sc[nb].int().item() - 127
        out[nb * H512_QUANT_BLOCK:(nb + 1) * H512_QUANT_BLOCK] = (
            fp8[nb * H512_QUANT_BLOCK:(nb + 1) * H512_QUANT_BLOCK] * (2.0 ** e))
    out[H512_NOPE:] = buf[blk][base + H512_NOPE:base + H512_TOKEN_STRIDE].view(torch.bfloat16).float()
    return out
...
def compare_to_triton(ref, hip, ctx) -> tuple:
    ...
    * indexer (head=128): whole-row mean (the FP8 single-scale / MXFP4 path is
    ...
    shape = ctx.shape
    nc = ctx.num_compress
    byte_pct = (ref.view(torch.uint8).int() == hip.view(torch.uint8).int()
                ).float().mean().item() * 100
    if shape.head_dim == 512:
        ...
        detail = f"byte%={byte_pct:.2f} nope_mean={nope_mean:.5f} rope_max={rope_max:.4f}"
    else:
        dmean = 0.0
        for ci in range(nc):
...
def _torch_indexer_row(ctx, P, r) -> torch.Tensor:
    ...
    for k in range(8):
        ...
        if pk < 0:
            valid.append(False)
            kv_l.append(torch.zeros(HEAD)); sc_l.append(torch.zeros(HEAD)); continue
        valid.append(True)
        blk = int(ctx.block_table[r, pk // sbs].item()); off = pk % sbs
        kv_l.append(state[blk, off, ho:ho + HEAD])
        sc_l.append(state[blk, off, SW + ho:SW + ho + HEAD] + ape[k % 4, ho:ho + HEAD])
    ...
    e = normed[64::2]; o = normed[65::2]                      # rope pairs (dims 64..127)
    cos_v = cs[idx]; sin_v = cs[32 + idx]
...
def _torch_mxfp4_dequant(row: torch.Tensor) -> torch.Tensor:
    ...
    odd_pen = torch.tensor([0., 1, 0, 1, 0, 1, 0, 1], device=row.device) * 1e-6
...
def mxfp4_oracle_diff(ctx, hip_cpu, max_check=512) -> float:
    ...
    """
    nc = ctx.num_compress
    bnd = [(p, r) for p, r, ks in zip(ctx.positions, ctx.token_to_req,
                                      ctx.kv_slot_mapping) if ks >= 0]
    dmean = 0.0
